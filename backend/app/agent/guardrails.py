...
1. [가장 중요 - 정량 수치 강제 고정]:
   ...
   - 표에 'N/A'나 '데이터 미제공'을 출력하지 마십시오. 모든 수치와 종목명은 제공된 팩트 데이터를 기반으로 100% 채워 넣으십시오.
2. [🚨 마크다운 표(Table) 줄바꿈 절대 원칙 - 렌더링 깨짐 방지]:
   - 모든 마크다운 표(수익성 분석표, 안정성 분석표, 성장성 분석표, 3개년 실적표 등)는 **절대로 한 줄에 이어 쓰지 마십시오!**
   - 표의 헤더 행, 구분선 행(`| :--- | :---: | ... |`), 각 데이터 행마다 **반드시 명확한 개행(줄바꿈, \\n)**을 넣어 마크다운 표가 브라우저에서 아름다운 테이블로 렌더링되게 하십시오.
3. [DART 공시 시점 기준]:
   - 공시 기준일은 반드시 **'2025년 정기 사업보고서(연간 결산) 및 2026년 최신 반기보고서'** 기준으로 표기하십시오. (과거 3Q 등으로 표기 금지)
4. [출력 형식 100% 준수]:
