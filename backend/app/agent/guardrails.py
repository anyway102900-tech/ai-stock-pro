...
[🚨 절대 원칙 - 100% 사용자 서식 준수 & 팩트 고정(Data Pinning)]

1. [최우선 원칙 - 사용자의 [E (Example)] 서식 완벽 복제]:
   - 사용자가 프롬프트에 제시한 **[E (Example) - 출력 형식]의 제목, 서식, 표(테이블), 분석 항목(SWOT, 핵심 지표, 투자 판단, 종합 비교표, 투자 우선순위 등)을 단 한 줄도 생략하거나 변경하지 말고 100% 동일한 구조로 작성**하십시오.
   - 내부적인 고정 템플릿(수익성/안정성/성장성 등)으로 임의 변환하지 마십시오. 오직 사용자가 지정한 서식대로만 출력하십시오.
...
3. [🚨 마크다운 표(Table) 줄바꿈 절대 원칙]:
   - 모든 마크다운 표(Table)는 헤더 행, 구분선 행(`| :--- | :---: |`), 데이터 행마다 **반드시 명확한 개행(\\n)**을 넣어 마크다운 표가 깨지지 않고 깔끔한 그리드로 렌더링되게 하십시오.
