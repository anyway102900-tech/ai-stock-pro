...
1. [가장 중요 - 정량 수치 강제 고정]:
   - '현재가', '52주 최고/최저', '시가총액', 'PER', 'PBR', '배당수익률', 'EPS', 'BPS', '3~4개년 연간 재무실적' 등의 정량 수치는 인터넷 검색이나 임의 추측을 쓰지 마십시오!
   - 반드시 아래 제공된 [1차 공인 팩트 데이터]에 명시된 숫자를 1원도 바꾸지 말고 100% 그대로 표와 본문에 기재하십시오.
   - 제공된 팩트 데이터에 없는 정보나 'N/A'로 표기된 항목은 **절대로 가짜 숫자를 지어내지 말고 반드시 'N/A' 또는 '확인 불가'로 솔직하게 표기**하십시오.
2. [🚨 마크다운 표(Table) 줄바꿈 절대 원칙 - 렌더링 깨짐 방지]:
   - 모든 마크다운 표(수익성 분석표, 안정성 분석표, 성장성 분석표, 3개년 실적표, 듀퐁 분해표 등)는 **절대로 한 줄에 이어 쓰지 마십시오!**
   - 표의 헤더 행(`| 구분 | 지표 |`), 구분선 행(`| :--- | :---: |`), 각 데이터 행(`| ROE | 15.2% |`)마다 **반드시 명확한 줄바꿈(\\n)**을 넣어 마크다운 표가 완벽한 직사각형 테이블로 렌더링되게 하십시오.
3. [DART 공시 시점 기준]:
   - 공시 기준일은 반드시 **'2025년 정기 사업보고서(연간 결산) 및 2026년 최신 반기보고서'** 기준으로 표기하십시오.
4. [출력 형식 100% 준수]:
   - 1. 수익성 분석 표, 2. 안정성 분석 표, 3. 성장성 분석 표, 4. 듀퐁 분해 표를 각각 독립된 정규 마크다운 표로 깔끔하게 분리하여 작성하십시오.
"""
